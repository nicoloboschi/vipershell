...
const isMd  = name => ['md','markdown','mdx'].includes((name ?? '').split('.').pop()?.toLowerCase() ?? '');
const isImg = name => ['png','jpg','jpeg','gif','webp','svg','ico','bmp'].includes((name ?? '').split('.').pop()?.toLowerCase() ?? '');
...
function FileBlock({ file, gitRoot, isFocused }) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 12px', background: '#161b22', borderBottom: (collapsed && !preview) ? 'none' : '1px solid #30363d' }}>
        ...
        {absPath && !file.isDeleted && (
          <button
            ...
            onClick={e => { e.stopPropagation(); setPreview(p => !p); setCollapsed(false); }}
...
function FileSidebar({ files, onJump, onOpenFile }) {
...
function CommitList({ sessionId, base, selected, onSelect }) {
  ...
  useEffect(() => {
    ...
    fetch(url)
      .then(r => r.json())
      .then(data => { setCommits(data); if (data.length > 0 && !selected) onSelect(data[0].hash); })
      .catch(() => setCommits([]))
    ...
    <div style={{ width: 260, flexShrink: 0, borderRight: '1px solid #30363d', overflowY: 'auto', background: '#0d1117' }}>
      ...
      {commits.map(c => (
        <div
          ...
          onMouseEnter={e => { if (selected !== c.hash) e.currentTarget.style.background = '#161b22'; }}
          onMouseLeave={e => { if (selected !== c.hash) e.currentTarget.style.background = 'transparent'; }}
        >
...
export default function GitDiffPane({ sessionId, onOpenFile }) {
        ...
        {showSidebar && (
          <div className="hidden md:flex flex-col" style={{ width: 240, flexShrink: 0, borderRight: '1px solid #30363d' }}>
            <FileSidebar
              files={files}
              onJump={jumpToFile}
              onOpenFile={onOpenFile && gitRoot ? (relPath) => onOpenFile(`${gitRoot}/${relPath}`) : null}
            />
          </div>
        )}
