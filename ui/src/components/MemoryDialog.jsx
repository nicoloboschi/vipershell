...
const TABS = ['Overview', 'Claude Code', 'Settings'];

const LLM_PROVIDERS = ['mock', 'openai', 'anthropic', 'groq', 'ollama', 'gemini', 'lmstudio', 'openai-codex'];
const NO_KEY_PROVIDERS = new Set(['mock', 'ollama', 'lmstudio', 'openai-codex']);

export default function MemoryDialog({ onClose }) {
  const [tab, setTab] = useState('Overview');
  const [mcpState, setMcpState] = useState('idle'); // idle | loading | ok | error
  const [mcpError, setMcpError] = useState('');

  const [cfg, setCfg] = useState(null);
  const [restartState, setRestartState] = useState('idle'); // idle | loading | ok | error
          ...
          {tab === 'Overview' && (
            <>
              <p className="text-sm text-muted-foreground leading-relaxed">
                ...
                <strong className="text-foreground font-semibold">Hindsight</strong> — a long-term
              ...
              <ul className="text-xs text-muted-foreground leading-loose list-disc pl-4">
                ...
                <li>No LLM required — extraction runs locally</li>
              </ul>
              <Button
                variant="outline"
                size="sm"
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      {cfg.llmProvider === 'mock' && (
                        <p className="text-xs text-muted-foreground">Mock mode — no LLM calls, chunks stored as-is.</p>
                      )}
                      {cfg.llmProvider === 'openai-codex' && (
                        <p className="text-xs text-muted-foreground">Uses <code className="bg-muted px-1 rounded">~/.codex/auth.json</code> — run <code className="bg-muted px-1 rounded">codex auth login</code> first.</p>
                      )}
                    </div>
