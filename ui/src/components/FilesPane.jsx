...
function getIcon(name, isDir, open = false) {
...
const isImage = name => ['png','jpg','jpeg','gif','webp','svg','ico','bmp'].includes(ext(name));
const isPdf   = name => ext(name) === 'pdf';
const isMd    = name => ['md','markdown','mdx'].includes(ext(name));
const isText  = name => !isImage(name) && !isPdf(name);
...
function Entry({ entry, selected, onSelect, onNavigate, gitStatus }) {
  ...
  return (
    ...
    >
      ...
      {status && (
        <span style={{ fontSize: 9, color: gitColor, fontWeight: 700, flexShrink: 0, fontFamily: '"Cascadia Code","JetBrains Mono",monospace' }}>
          {GIT_LABELS[status]}
        </span>
      )}
...
function FileViewer({ path, highlightQuery, highlightLine }) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderBottom: '1px solid #30363d', background: '#161b22', flexShrink: 0 }}>
        ...
        {textFile && (
          <div style={{ display: 'flex', border: '1px solid #30363d', borderRadius: 5, overflow: 'hidden' }}>
            {[
              { id: 'preview', icon: <Eye size={11} />,    label: mdFile ? 'Preview' : 'View' },
            ...
            <div style={{ flex: 1, overflow: 'auto', fontFamily: '"Cascadia Code","JetBrains Mono",monospace', fontSize: 12 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <tbody>
                  {content.split('\n').map((line, i) => {
                    const lineNum = i + 1;
                    const isTarget = highlightLine && lineNum === highlightLine;
                    return (
...
function SearchResult({ result, cwd, isActive, onClick, query }) {
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {highlightText(result.text.trim())}
      </div>
    </div>
  );
}

export function SearchPanel({ sessionId, onOpenFile }) {
...
export default function FilesPane({ sessionId, openFileRef, onFileSelect, highlightQuery, highlightLine }) {
  ...
  const [gitStatus,    setGitStatus]    = useState(null); // { [absPath]: 'modified'|'added'|... }
  const [isDragOver,   setIsDragOver]   = useState(false);
  const [uploadMsg,    setUploadMsg]    = useState(null);
  const draggingRef = useRef(false);

  const browse = useCallback(async (targetPath, { autoReadme = false } = {}) => {
    setLoading(true);
    try {
      ...
      if (autoReadme) {
        const readme = data.entries.find(e => !e.isDir && /^readme\.md$/i.test(e.name));
  ...
  useEffect(() => {
    ...
    openFileRef.current = async (filePath) => {
  ...
  const fileList = (
    <>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {entries.map(e => (
          <Entry key={e.path} entry={e} selected={selectedFile} onSelect={selectFile} onNavigate={browse} gitStatus={gitStatus} />
        ))}
      </div>
    </>
  );
      ...
      <div className="hidden md:flex flex-col flex-1 min-h-0">
        ...
        <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
          <div style={{ width: sidebarWidth, flexShrink: 0, overflowY: 'auto', overflowX: 'hidden', position: 'relative' }}>
            {loading && <div style={{ padding: '8px 12px', color: '#6e7681', fontSize: 12 }}>Loading…</div>}
            {entries.map(e => (
              <Entry key={e.path} entry={e} selected={selectedFile} onSelect={setSelectedFile} onNavigate={browse} gitStatus={gitStatus} />
            ))}
            {/* Resize handle */}
            <div
